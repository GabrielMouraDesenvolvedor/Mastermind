...
function init() {
    ...
    createRandomCode(); // Gera um novo código aleatório no início de um novo jogo.
}
...
function createCorrectionArray(input_colors_arr) {
    ...
    // Verifica cores erradas
    for (let i in input_colors_arr) {
        if (correction_Array[i] !== 'green') {
            if (input_colors_arr[i] !== random_code[i]) {
